...
def user_sexo(update):
    db = client.users
    posts = db.users 
    posts.update({"user_id":update.message.chat.id},{"$addToSet":{"Sexo":update.message.text}})#update.message.text
    if update.message.text == "Chico":
       posts.update({"user_id":update.message.chat.id},{"$addToSet":{"Codigo_sexo":0}})
    else:
       posts.update({"user_id":update.message.chat.id},{"$addToSet":{"Codigo_sexo":1}})
